...
namespace Agot2Client
{
    public class GamePortal : MyNotifyObj
    {
        ...
        //public bool PublicDonate { get; set; }
        public void Donate()
        {
            try
            {
                ...
                {
                    Process.Start(string.Format("http://lantsev1981.pro/Donate/donate_ru.html?lang=ru&id={0}", User.Login));
                }
                else
                {
                    Process.Start(string.Format("http://lantsev1981.pro/Donate/donate_en.html?lang=en&id={0}", User.Login));
                }
            }
            catch (Exception exp)
            {
        ...
        public void VkMessageSend(string login, string text)
        {
            if (string.IsNullOrEmpty(text) || VkAuthWindow.VKAPI == null)
            {
                return;
            }
            ...
            VkAuthWindow.VKAPI.MessagesSend(text, login);
        }
